...
class CreateGroupsGenericByUbication(GenericUseCase):
        ...
        try:
            ...
            for zona in zonas:
                ...
                for ubi in ubication:
                    ...
                    df_generic_by_ubication = self.df[(self.df[ColumnsNameExcelConfigISmart.ubicacion.value] == ubi) &
                                                (self.df[ColumnsNameExcelConfigISmart.zonas.value] == zona) & 
                                                (self.df[ColumnsNameExcelConfigISmart.domain.value] == name_domain)] 
                    
                    name_group_generic_by_ubication = self.name_of_group.value +" "+ ubi
                    unique_id_generic = GroupsUtilUseCase.build_unique_id(name_file_yaml +"_" +name_group_generic_by_ubication)
                    
                    df_generic_by_ubi =  df_generic_by_ubication[df_generic_by_ubication[ColumnsNameExcelConfigISmart.name_entity.value].isin([name_entity_ismart])]
                    
                    dict_df_generic_by_ubi = {}

                    if name_domain == DomainEntitiesIsmartEnum.sensor.value:
                    
                        dict_df_generic_by_ubi = GroupsUtilUseCase.build_dict_group_sensor(df_generic_by_ubi, name_group_generic_by_ubication, unique_id_generic,"mean")

                    if name_domain == DomainEntitiesIsmartEnum.cover.value:

                        dict_df_generic_by_ubi = GroupsUtilUseCase.build_dict_group_cover(df_generic_by_ubi, name_group_generic_by_ubication, unique_id_generic)

                    if dict_df_generic_by_ubi:

                        name_file =  name_file_yaml +"_"+ StringUtilUseCase.convert_string_lower_case(name_entity_ismart)+"_" + StringUtilUseCase.convert_string_lower_case(ubi) + '.yaml'
                        path_save_yaml = PathsIsmartUseCase.path_join_any_directores([self.path_ismart_principal,'Zonas', zona,'Ubicacion', ubi, 'Integraciones','grupos',self.name_of_group.value])
                        FolderCreator.execute(path_save_yaml)
                        YamlUtilUseCase.save_file_yaml(PathsIsmartUseCase.path_join_any_directores([path_save_yaml, name_file]),dict_df_generic_by_ubi )
