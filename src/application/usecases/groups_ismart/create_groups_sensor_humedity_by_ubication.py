...
class CreateGroupsSensorHumedityByUbication(GenericUseCase):
    ...
    async def execute(self) -> pd.DataFrame:
        try:
            ...
            df_groups_sensor_humedity_by_ubi = GroupsUtilUseCase.build_df_empty_to_build_groups()
            ...
            for zona in zonas:
                ...
                for ubi in ubication:
                    ...
                    name_group_humedity_by_ubication = NameOfGroupEnum.sensor_humedad.value +" "+ ubi
                    unique_id_humedity = GroupsUtilUseCase.build_unique_id(name_file_yaml + name_group_humedity_by_ubication)
                    
                    df_sensor_humedity_by_ubi =  df_sensor_by_ubication[df_sensor_by_ubication[ColumnsNameExcelConfigISmart.name_entity.value].isin([NameEntitiesIsmartEnum.Humedad.value])]
                    dict_df_sensor_humedity_by_ubi = GroupsUtilUseCase.build_dict_group_sensor(df_sensor_humedity_by_ubi, name_group_humedity_by_ubication, unique_id_humedity,"mean")


                    if dict_df_sensor_humedity_by_ubi:

                        name_file =  name_file_yaml + StringUtilUseCase.convert_string_lower_case(NameEntitiesIsmartEnum.Humedad.value)+"_" + StringUtilUseCase.convert_string_lower_case(ubi) + '.yaml'
                        ...
                        YamlUtilUseCase.save_file_yaml(PathsIsmartUseCase.path_join_any_directores([path_save_yaml, name_file]),dict_df_sensor_humedity_by_ubi )

                        row_df_sensor_humedity_by_ubi = {
                                                                NameColumnDfGroupEnum.title.value:NameTitlesIsmartEnum.humedad_por_ubicacion.value, 
                                                                NameColumnDfGroupEnum.entity.value:'sensor.'+unique_id_humedity,
                                                                NameColumnDfGroupEnum.name_.value:name_group_humedity_by_ubication, 
                                                                NameColumnDfGroupEnum.icon.value:'mdi:home-thermometer', 
                                                                NameColumnDfGroupEnum.tap_action.value:'none'
                                                            }
                        
                        
                        df_groups_sensor_humedity_by_ubi = df_groups_sensor_humedity_by_ubi.append(row_df_sensor_humedity_by_ubi, ignore_index=True)

            
            return df_groups_sensor_humedity_by_ubi
        
        except Exception as exception:
            raise ErrorHandlingUtils.application_error("Error al crear el archivo group de sensores Humedad por Ubicación", exception)
